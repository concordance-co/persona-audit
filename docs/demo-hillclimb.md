...
and extends `docs/demo-dataset-build-plan.md`; read that first.
...
3. **Fixed user turns break coherence as tracks diverge.** The user side was
   ...
   the reply (authored seeds in `backend/demo/seeds.py` follow this; apply
...
```text
backend/demo/personas.py        versioned prompts (sol_v1, marrow_v1, control_v1)
backend/demo/seeds.py           staged seed sets + ESConv loader + validator
backend/demo/rounds.py          per-turn round building / result ingestion
backend/demo/transcript_qa.py   contrast guardrails (length, hedges, lexicon)
backend/demo/normalize.py       histories -> AuditTrace rows (required metadata)
backend/demo/separation.py      paired effect sizes, confound check, gates
backend/demo/state.py           artifacts/demo_hillclimb/state.json
backend/workflows/demo_generation.py  one round per run, batched all seeds x tracks
backend/workflows/demo_scoring.py     tau2 scoring surfaces over demo traces
backend/scripts/demo_hillclimb.py     driver CLI
```
...
Defined in `backend/demo/separation.py` (`DEFAULT_GATES`):
...
Transcript QA (`backend/demo/transcript_qa.py`) checks the build plan's
...
3. If gates fail, sharpen **one** persona: add a new version in
   `backend/demo/personas.py` (never edit an existing version), update
...
docs/demo-hillclimb.md, then stop if frozen". Budget note: one Stage 1
...
- [ ] `rows_from_score_artifact` in `backend/demo/separation.py` matches the
...
- [ ] `max_model_len` 4096 still fits stage-1 final rounds (bump
      `BEHAVIOR_AUDIT_DEMO_GENERATION_MAX_MODEL_LEN` if prompts grow).
