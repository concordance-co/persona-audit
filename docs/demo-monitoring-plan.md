...
(`backend/demo/checkin.py:escalator_brief`). The brief allows exactly one
move: read `separation.json` + `qa_report.json`, author ONE new prompt
version for ONE track in `backend/demo/personas.py`, point `prompt_ids` at it
...
- **Unset by default:** without `DEMO_HILLCLIMB_ESCALATOR_CMD`, tick prints
  ...
  docs/demo-hillclimb.md. Set the variable only when you want true autonomy.
- **Verifiable:** every escalation is one commit-sized diff (a new
  PersonaPrompt version) — easy to review in `git diff backend/demo/personas.py`.
...
- `git diff backend/demo/personas.py` — every prompt revision the escalator made
