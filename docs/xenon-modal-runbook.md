...
backend/scripts/run_xenon_workflow.sh runs --file backend/workflows/demo_generation.py
backend/scripts/run_xenon_workflow.sh show --run-id wr_...
backend/scripts/run_xenon_workflow.sh resume --file backend/workflows/demo_generation.py --latest-failed
...
| --- | --- | --- |
| `backend/workflows/demo_generation.py` | Demo dataset generation, one round per run (Sol/Marrow/control) | `generation_gpu` |
| `backend/workflows/demo_scoring.py` | Product scoring surfaces over normalized demo traces | `capture_gpu` |
...
hand — drive them with `backend/scripts/demo_hillclimb.py`
(see docs/demo-hillclimb.md).
...
XENON_WORKSPACE_ROOT=../xenon uv run python -m pipelines_v2.cli workflow run \
  --file backend/workflows/demo_generation.py --logging INFO
```
...
1. **One run, many examples.** Never loop `workflow run` over examples. Put
   ...
   (`BEHAVIOR_AUDIT_DEMO_GENERATION_MAX_NUM_SEQS`, default 16).
...
3. **Caches live on persistent volumes.** Model weights load from the
   `yora-models` volume at `/models` (`model_path_root`, no HF download), and
...
6. **Tune with env vars, not edits.** GPU type, timeouts, vLLM knobs, and
   ...
   (`BEHAVIOR_AUDIT_DEMO_GENERATION_*`, `BEHAVIOR_AUDIT_TAU2_*`,
   `BEHAVIOR_AUDIT_HERMES_*`).

## Generation (Demo Dataset)

`backend/workflows/demo_generation.py` implements the flow in
`docs/demo-dataset-build-plan.md`: fixed user turns, three system-prompt
...
`backend/scripts/demo_hillclimb.py generate` builds each round's examples from
`backend/demo/` (personas, seeds, per-track histories), writes a round file,
and points `BEHAVIOR_AUDIT_DEMO_ROUND_FILE` at it; without that env var the
...
`/data/artifacts/behavior_audit_demo_generation_v1/<artifact_id>/` on the
`xenon-data` Modal volume; the driver pulls them via `ModalVolumeStore` into
...
3. Score artifacts are pulled and uploaded with
   ...
   `upload_hermes_scores_to_neon.py`, which read `result.json` from the
   `xenon-data` volume and write the `behavior_audit_*_score_*` tables.

For the demo dataset, `backend/workflows/demo_scoring.py` reuses the Tau2
...
`BEHAVIOR_AUDIT_DEMO_TRACES_FILE`), carrying `track`/`paired_group_id` labels
through to the score artifacts for the separation metrics in
`backend/demo/separation.py`.
