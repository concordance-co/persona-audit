...
SUMMARY_TABLE = "persona_audit_score_summaries"
SUPPLEMENTAL_SCORE_DIR = DATA_ROOT / "supplemental_scores"
SCORE_SUMMARY_DIR = DATA_ROOT / "score_summaries"
...
def main() -> None:
    ...
    supplemental_payloads = (
        [] if args.skip_supplemental_scores else load_supplemental_score_payloads(args.supplemental_dir)
    )
    ...
    with psycopg.connect(database_url, autocommit=False, row_factory=dict_row) as conn:
        ...
        for payload in supplemental_payloads:
            ...
            ensure_tables(conn, run_table=run_table, score_table=score_table)
...
def load_supplemental_score_payloads(root: Path = SUPPLEMENTAL_SCORE_DIR) -> list[dict[str, Any]]:
    ...
    for path in sorted(root.glob("*.json")):
        ...
        if not isinstance(rows, list):
            continue
        payloads.append(
            {**payload, "rows": [dict(row) for row in rows if isinstance(row, Mapping)], "source_path": str(path)}
        )
    return payloads
...
def replace_trace_rows(
    ...
    *,
    ...
    conn.execute(
        sql.SQL("DELETE FROM {table} WHERE provider_id = ANY(%s)").format(table=trace_identifier), (providers,)
    )
    copy_rows(
        conn,
        trace_table,
...
def ensure_score_run(conn: psycopg.Connection, *, run_table: str, payload: Mapping[str, Any]) -> None:
    ...
    existing = conn.execute(
        sql.SQL("SELECT run_id FROM {table} WHERE run_id = %s").format(
            table=sql.Identifier(_safe_identifier(run_table))
        ),
        (run_id,),
    ).fetchone()
    if existing:
    ...
    conn.execute(
        ...
        (
            run_id,
            "persona_audit_local_supplemental_scores",
            provider_id,
            source,
            "local",
            ...
            "",
            str(
                payload.get("projection", {}).get("trait_vector_repo")
...
def replace_score_summaries(
    conn: psycopg.Connection, *, summary_table: str, payloads: Sequence[Mapping[str, Any]]
) -> None:
    summary_identifier = sql.Identifier(_safe_identifier(summary_table))
    for payload in payloads:
        conn.execute(
            sql.SQL("DELETE FROM {table} WHERE run_id = %s").format(table=summary_identifier), (payload["run_id"],)
        )
        conn.execute(
            sql.SQL(
                """
            ...
            (
                payload["run_id"],
                payload.get("kind") or "persona_audit_score_summary",
                payload.get("version"),
...
def copy_rows(
    ...
    with conn.cursor().copy(
        ...
        for row in rows:
            copy.write_row(tuple(copy_value(row.get(column)) for column in columns))
