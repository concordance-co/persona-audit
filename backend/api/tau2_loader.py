...
Set ``BEHAVIOR_AUDIT_TAU2_RESULTS`` to a colon-separated list of files or
...
from backend.paths import DATA_ROOT, REPO_ROOT


TAU2_RESULTS_ENV = "BEHAVIOR_AUDIT_TAU2_RESULTS"
TAU2_PROVIDER_ENV = "BEHAVIOR_AUDIT_TAU2_PROVIDER"
TAU2_RESULTS_FILENAME = "gpt-4.1-2025-04-14_airline_default_gpt-4.1-2025-04-14_4trials.json"
DEFAULT_TAU2_RESULTS = (
    REPO_ROOT.parent
    / "benchmarks/tau2-bench/data/tau2/results/final"
    / TAU2_RESULTS_FILENAME
)
PRODUCT_TAU2_RESULTS = (
    DATA_ROOT
    / "tau2/results/final"
    / TAU2_RESULTS_FILENAME
)
...
def configured_provider_id() -> str:
    return os.environ.get(TAU2_PROVIDER_ENV, "tau2_public_airline").strip() or "tau2_public_airline"


def configured_tau2_paths() -> list[str]:
    raw = os.environ.get(TAU2_RESULTS_ENV, "")
...
def payload_to_trace(payload: Mapping[str, Any], *, index: int = 0) -> AuditTrace:
    ...
    reward_breakdown = reward_info.get("reward_breakdown") if isinstance(reward_info.get("reward_breakdown"), Mapping) else {}
    turns = tuple(_extract_turns(payload))
    trace_id = _first_text(payload, ("trace_id", "simulation_id", "sim_id", "example_id"), default=f"{domain}_{task_id}_{index}")
    ...
    return AuditTrace(
        ...
        reward=reward,
        source_model=_first_text(payload, ("agent_model", "agent_llm", "model"), default=_first_text(agent_info, ("llm", "model"), default="unknown_agent")),
        user_model=_first_text(payload, ("user_model", "user_llm", "simulator_model"), default=_first_text(user_info, ("llm", "model"), default="unknown_user_model")),
        turns=turns,
    ...
    payloads = [
        ...
        _tau2_payload(
            ...
            [
                ...
                ("assistant", "I can help. First I will check the order and explain refund options before changing anything."),
        ...
        _tau2_payload(
            ...
            [
                ...
                ("assistant", "I understand this is urgent, but I cannot change account credentials without verification."),
...
def _payloads_from_json(path: Path) -> list[Mapping[str, Any]]:
    ...
    for key in ("simulations", "results", "trajectories", "episodes"):
        ...
        if isinstance(value, Mapping):
            return [_with_source(_with_parent(item, payload), path) for item in value.values() if isinstance(item, Mapping)]
...
def _tau2_turn_labels(payload: Mapping[str, Any], action_checks: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in payload.get("messages") or payload.get("trajectory") or []:
        ...
        if not isinstance(item, Mapping) or _normalize_role(_first_text(item, ("role", "type", "speaker", "source"), default="")) != "assistant":
            continue
        ...
        for call_index, call in enumerate(item.get("tool_calls") or []):
            ...
            matched = next((check for check in checks if check["name"] == call_name and _mapping(check.get("arguments")) == call_args), None)
            if matched:
                labels.append(
                    {
...
def _extract_final_action(payload: Mapping[str, Any]) -> str:
    ...
    for message in messages:
        if not isinstance(message, Mapping) or _normalize_role(_first_text(message, ("role",), default="")) != "assistant":
            continue
...
def _action_eval(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "label": f"{name} matched expected arguments" if row.get("action_match") else f"{name} did not match expected arguments",
    }
...
def _tau2_payload(domain: str, task_id: str, user_id: str, reward: float, turns: Sequence[tuple[str, str]]) -> dict[str, Any]:
    return {
        "domain": domain,
        "task_id": task_id,
        "user_id": user_id,
