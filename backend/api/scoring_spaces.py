...
from papers.voice.assistant_axis.assets import load_asset_manifest as load_assistant_manifest
from papers.voice.emotions.assets import emotion_concepts, load_asset_manifest as load_emotion_manifest
    ...
    *,
    ...
    return {
        ...
        "spaces": [
            ...
            _emotion_space(),
            *([] if str(provider_id).startswith("hermes") else [_high_stakes_probe_space()]),
        ],
    }
...
def _emotion_space() -> dict[str, Any]:
    ...
    return {
        ...
        "xenon_specs": ["EmotionPrecomputedVectorSpaceSpec", "EmotionScoreSpec"],
...
def _high_stakes_probe_space() -> dict[str, Any]:
    artifacts = [dict(artifact) for artifact in HIGH_STAKES_PERSISTED_PROBES]
    ...
    return {
        ...
        "modal_roots": sorted({str(artifact["target_root"]) for artifact in artifacts}),
        "artifacts": artifacts,
        "verification": {
            "corrected_adapted_manifest": (
                "public-demo-probe-export-placeholder"
            ),
        ...
        "xenon_specs": ["PersistedProbeImportSpec", "PersistedProbeInferenceSpec"],
        "requires": [
            "full-trace residual capture at layer 31",
            "Modal access to /data/artifacts/high_stakes_interactions_persisted_probes_v1 and _v2",
        ],
    }


def _render_trace_with_span(turns: Sequence[AuditTurn], *, target_turn: AuditTurn) -> tuple[str, dict[str, int], dict[str, int] | None]:
    parts: list[str] = []
