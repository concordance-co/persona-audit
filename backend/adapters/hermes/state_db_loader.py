...
Resolution order:
1. ``PERSONA_AUDIT_HERMES_STATE_DB`` or ``HERMES_SIDECAR_STATE_DB`` env var, if set.
...
STATE_DB_ENV = "PERSONA_AUDIT_HERMES_STATE_DB"
LEGACY_STATE_DB_ENV = "HERMES_SIDECAR_STATE_DB"
PROVIDER_ENV = "PERSONA_AUDIT_HERMES_PROVIDER"
LEGACY_PROVIDER_ENV = "HERMES_SIDECAR_PROVIDER"
INCLUDE_ARCHIVED_ENV = "PERSONA_AUDIT_HERMES_INCLUDE_ARCHIVED"
...
def configured_provider_id() -> str:
    return (env_value(PROVIDER_ENV) or env_value(LEGACY_PROVIDER_ENV) or "hermes_local").strip() or "hermes_local"


def configured_state_db_path() -> Path | None:
    explicit = (env_value(STATE_DB_ENV) or env_value(LEGACY_STATE_DB_ENV) or "").strip()
    if explicit:
...
def _include_archived() -> bool:
    raw = env_value(INCLUDE_ARCHIVED_ENV) or env_value(LEGACY_INCLUDE_ARCHIVED_ENV) or "0"
