...
def default_state_db_path() -> Path:
    ...
    Defaults to ``~/.hermes/state.db`` but honors ``BEHAVIOR_AUDIT_HERMES_STATE_DB``
    and the legacy ``HERMES_SIDECAR_STATE_DB`` so
    demos and tests can point at a synthetic database without touching a real
    ``~/.hermes`` directory.
    """

    override = os.getenv("BEHAVIOR_AUDIT_HERMES_STATE_DB", "").strip() or os.getenv("HERMES_SIDECAR_STATE_DB", "").strip()
    if override:
