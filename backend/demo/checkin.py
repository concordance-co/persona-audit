...
from backend.demo.state import HillClimbState

STALL_WINDOW = 2

_DRIVER = "uv run python -m backend.scripts.demo_hillclimb"
...
def render_markdown(
    ...
    *,
    ...
    if recent:
        trend = ", ".join(
            f"#{entry.get('iteration')}: {entry.get('objective')}" for entry in recent
        )
...
1. Read artifacts/demo_hillclimb/separation.json, artifacts/demo_hillclimb/qa_report.json, and backend/demo/personas.py.
2. Pick the single track (sol or marrow) most responsible for the failure and author ONE new PersonaPrompt version for it in backend/demo/personas.py: copy the latest version for that track, bump version and prompt_id (sol_v1 -> sol_v2), register it in PROMPTS. Never edit or delete an existing version. Aim the revision directly at the failing checks and surfaces named in the reports.
...
Hard limits: never run generate, score, run-iteration, tick, or any Modal/xenon command; never edit seeds, gates, QA thresholds, tests, or any file other than backend/demo/personas.py and artifacts/demo_hillclimb/state.json; never change more than one track's prompt. The next tick tests your prompt automatically."""
