...
from backend.demo import checkin, normalize, rounds, run_lock, separation, transcript_qa
from backend.demo.personas import PROMPTS, TRACKS, latest_prompt, latest_prompts
from backend.demo.seeds import STAGE0_SEEDS, STAGE1_SEEDS, seeds_for_stage, validate_seeds
from backend.demo.state import HillClimbState, load_state, save_state
...
def test_round_examples_condition_only_on_own_track_history():
    ...
    assert len(turn0) == 3
    fake_rows = [
        {"example_key": ex["key"], "generated_text": f"reply from {ex['labels']['track']}"}
        for ex in turn0
    ]
...
def test_normalize_round_trip(tmp_path):
    prompts = latest_prompts()
    histories: rounds.Histories = {
        (STAGE0_SEEDS[0].seed_id, track): [f"{track} t0", f"{track} t1"] for track in TRACKS
    }
    traces = normalize.traces_from_histories(
        STAGE0_SEEDS, prompts, histories, generation_params={"temperature": 0.45}
    )
    assert len(traces) == 3
    for trace in traces:
...
def _iterated_state(
    objectives: list[float], *, passed: bool = False, qa_passed: bool | None = None
) -> HillClimbState:
...
def test_checkin_separation_pass_with_qa_fail_escalates_instead_of_advancing():
    ...
    decision = checkin.decide(
        state, has_transcripts=True, has_separation=True, qa_failing_tracks=["marrow"]
    )
...
def test_checkin_escalates_on_stall_and_names_qa_track():
    ...
    decision = checkin.decide(
        failing, has_transcripts=True, has_separation=True, qa_failing_tracks=["marrow"]
    )
    assert decision["action"] == "ESCALATE_PROMPT"
    assert "marrow" in decision["reason"]
...
def test_tick_escalation_requires_configured_escalator(tmp_path, monkeypatch):
    from backend.scripts import demo_hillclimb as driver
