...
from backend.api import audit_data


def test_session_turn_deviations_threads_provider_to_score_rows(monkeypatch) -> None:
    calls: list[str | None] = []

    monkeypatch.setattr(audit_data, "PERSONA_VECTOR_NAMES", ("assistant_axis",))
    monkeypatch.setattr(audit_data, "OVERVIEW_PERSONA_VECTORS", ("assistant_axis",))
    monkeypatch.setattr(audit_data, "OVERVIEW_EMOTION_CLUSTER_VECTORS", ())
    monkeypatch.setattr(audit_data, "_persona_coordinates", lambda: ("assistant_axis",))

    def fake_score_rows_for_coordinates(coordinates: tuple[str, ...], provider: str | None = None) -> list[dict[str, Any]]:
    ...
    monkeypatch.setattr(audit_data, "score_rows_for_coordinates", fake_score_rows_for_coordinates)
    ...
    rows = audit_data._session_turn_deviations("provider_trace", records, provider="hermes")
...
def test_outlier_turn_series_threads_provider_to_score_rows(monkeypatch) -> None:
    ...
    series = audit_data._outlier_turn_series(records, outliers, provider="hermes")
