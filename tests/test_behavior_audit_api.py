...
def test_behavior_audit_report_exposes_tau2_modules() -> None:
    ...
    assert overview["correlation_n"] >= 1
    if payload["score_surface"].get("available"):
        ...
        emotion_correlation = [abs(row["pass_correlation"]) for row in emotion_rows if row.get("pass_correlation") is not None]
...
def test_behavior_audit_product_analytics_is_separate_from_report() -> None:
    ...
    assert payload["kind"] == "behavior_audit_product_analytics"
    assert payload["trace_count"] >= 8
    ...
    if persona.get("available"):
        ...
        assert set(inventory["negative_affect"]["source_clusters"]) >= {"Hostile Anger", "Fear and Overwhelm", "Despair and Shame"}
        ...
        assert all(coordinate.startswith("emotion_cluster__") for coordinate in inventory["confidence_affect"]["coordinates"])
...
def test_behavior_audit_sessions_include_drilldown_payloads() -> None:
    ...
    assert payload["session_analytics"]["emotion_clusters"]
    assert payload["session_analytics"]["vector_deviations"]
    assert {"task_group", "final_action", "task_group_summary"}.issubset(payload["session_analytics"]["product_groups"])
    assert {"session", "expected_mean", "global_percentile"}.issubset(payload["session_analytics"]["vector_deviations"][0])
...
def test_behavior_audit_score_spaces_expose_real_provider_and_precomputed_assets() -> None:
    ...
    assert payload["xenon_input"]["record_count"] >= payload["provider"]["trace_count"]
    ...
    assert any(
        artifact["artifact_id"] == "probe_1_bb998787"
        and artifact["training_mode"] == "staged_finetune"
        and artifact["stage_epochs"] == [1, 12]
        for artifact in spaces["high_stakes_probe"]["artifacts"]
    )
    assert "PersistedProbeInferenceSpec" in spaces["high_stakes_probe"]["xenon_specs"]
