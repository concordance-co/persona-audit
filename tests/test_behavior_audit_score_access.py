...
from backend.api import neon_scores
from backend.api import score_cache


def test_score_provider_run_ids_and_identifier_validation(monkeypatch) -> None:
    monkeypatch.delenv("BEHAVIOR_AUDIT_SCORE_RUN_ID", raising=False)

    assert neon_scores._score_run_id("tau2") == neon_scores.DEFAULT_RUN_ID
    assert neon_scores._score_run_id("hermes") == neon_scores.HERMES_DEFAULT_RUN_ID
    assert neon_scores._safe_identifier("valid_table_123") == "valid_table_123"

    try:
        neon_scores._safe_identifier("bad;drop")
...
def test_supplemental_inventory_merge_preserves_existing_and_appends_family() -> None:
    merged = neon_scores._merge_supplemental_inventory(
        {"available": True, "run_id": "run", "families": [{"score_family": "emotion", "coordinate_count": 2, "row_count": 10}]},
        [
...
def test_score_rows_for_coordinates_uses_supplemental_without_database(monkeypatch) -> None:
    neon_scores.score_rows_for_coordinates.cache_clear()
    monkeypatch.setattr(neon_scores, "_database_url", lambda: None)
    monkeypatch.setattr(
        neon_scores,
    ...
    rows = neon_scores.score_rows_for_coordinates(("assistant_axis_trait__calm",), run_id="run")
...
def test_supplemental_rows_are_not_duplicated_when_db_already_has_them() -> None:
    ...
    rows = neon_scores._append_missing_supplemental_rows([db_row], [supplemental_duplicate, supplemental_new])
...
def test_score_summary_from_table_row_prefers_materialized_payload() -> None:
    row = {
        ...
        "version": neon_scores.SCORE_SUMMARY_CACHE_VERSION,
    ...
    summary = neon_scores._score_summary_from_table_row(row, "run")
...
def test_score_summary_from_table_row_rejects_wrong_run_or_version() -> None:
    row = {
        "run_id": "run",
        "kind": "behavior_audit_score_summary",
        "version": neon_scores.SCORE_SUMMARY_CACHE_VERSION - 1,
        "score_inventory": {},
        "score_surface": {},
        "module_scores": [],
        "payload": None,
    ...
    assert neon_scores._score_summary_from_table_row(row, "run") is None
    assert neon_scores._score_summary_from_table_row({**row, "version": neon_scores.SCORE_SUMMARY_CACHE_VERSION}, "other") is None


def test_score_summary_cache_honors_configured_file_or_directory(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("BEHAVIOR_AUDIT_SCORE_SUMMARY_CACHE", str(tmp_path))
    assert score_cache._score_summary_cache_path("run/id").name == "run_id.json"

    explicit_file = tmp_path / "summary.json"
    monkeypatch.setenv("BEHAVIOR_AUDIT_SCORE_SUMMARY_CACHE", str(explicit_file))
    payload = {
        "kind": "behavior_audit_score_summary",
