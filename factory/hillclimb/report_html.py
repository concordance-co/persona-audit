...
def _trend_svg(history: Sequence[Mapping[str, Any]], gate: float) -> str:
    points = [
        (int(entry["iteration"]), float(entry["objective"])) for entry in history if entry.get("objective") is not None
    ]
    if not points:
...
def _history_table(history: Sequence[Mapping[str, Any]]) -> str:
    ...
    for entry in reversed(list(history)[-10:]):
        ...
        sep_cell = (
            '<span class="pass">pass</span>' if entry.get("separation_passed") else '<span class="fail">fail</span>'
        )
...
def render(
    *,
    ...
    gates_panel = f"<ul class='reason'>{reason_list}</ul>" if reasons else '<div class="pass">all gates passing</div>'
...
if __name__ == "__main__":  # pragma: no cover - manual preview helper
    print(json.dumps({"module": "factory.hillclimb.report_html", "refresh": REFRESH_SECONDS}))
