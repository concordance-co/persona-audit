...
factory/hillclimb/personas.py) — is delegated to a bounded escalator agent by
`tick`, the autonomous heartbeat (docs/demo-monitoring-plan.md).

Usage (from the repo root):

    uv run python -m factory.scripts.demo_hillclimb status
    uv run python -m factory.scripts.demo_hillclimb tick          # cron this
    uv run python -m factory.scripts.demo_hillclimb checkin --log # digest only
    uv run python -m factory.scripts.demo_hillclimb run-iteration --stage 1
    uv run python -m factory.scripts.demo_hillclimb generate --stage 0
    uv run python -m factory.scripts.demo_hillclimb qa
    uv run python -m factory.scripts.demo_hillclimb normalize
    uv run python -m factory.scripts.demo_hillclimb score
    uv run python -m factory.scripts.demo_hillclimb evaluate
"""
...
from factory.hillclimb import checkin as demo_checkin
from factory.hillclimb import normalize as demo_normalize
from factory.hillclimb import report_html as demo_report
from factory.hillclimb import rounds as demo_rounds
from factory.hillclimb import run_lock as demo_run_lock
from factory.hillclimb import separation as demo_separation
from factory.hillclimb import transcript_qa
from factory.hillclimb.personas import prompt as persona_prompt
from factory.hillclimb.seeds import seeds_for_stage, validate_seeds
from factory.hillclimb.state import HILLCLIMB_ROOT, STATE_PATH, HillClimbState, load_state, save_state
from factory.workflows.demo_generation import (
    GENERATION_PARAMS,
    ROUND_FILE_ENV,
)
from factory.workflows.demo_generation import (
...
from factory.workflows.demo_scoring import (
...
def _localize_artifact(artifact_root: str, artifact_id: str) -> Path:
    return localize_artifact(artifact_root, artifact_id, HILLCLIMB_ROOT / "modal_cache")
...
def cmd_generate(ns: argparse.Namespace) -> int:
    ...
    if state.frozen and not ns.allow_frozen:
        print(
            "Prompts are frozen (Stage 2 passed). Rerun with --allow-frozen for scale runs; do not tune prompts.",
            file=sys.stderr,
        )
        return 2
    ...
    for turn_index in range(total_rounds):
        examples = demo_rounds.build_round_examples(seeds, prompts, histories, turn_index, stage=ns.stage)
        if not examples:
            break
...
def cmd_evaluate(_: argparse.Namespace) -> int:
    ...
    if not merged:
        print(
            ...
            "factory/hillclimb/separation.py:rows_from_score_artifact to its layout.",
            file=sys.stderr,
        )
        return 2
...
def _escalate_prompt(state: HillClimbState, reason: str) -> int:
    ...
    if marker and marker.get("iteration") == state.iteration and marker.get("prompt_ids") == dict(state.prompt_ids):
        print(
    ...
    if dict(new_state.prompt_ids) != dict(state.prompt_ids):
        print(f"escalator bumped prompts to {new_state.prompt_ids}; the next tick will run the iteration.")
        return 0
...
def _write_report() -> None:
    ...
    if TICK_LOG_PATH.exists():
        log_tail = "\n".join(TICK_LOG_PATH.read_text(encoding="utf-8").splitlines()[-40:])
    escalation = _read_json(ESCALATION_MARKER_PATH) if ESCALATION_MARKER_PATH.exists() else None
...
def _write_snapshot(label: str, note: str = "") -> Path:
    ...
    manifest = {
        ...
        "files": copied,
        "note": note,
        "provenance": "Generated via factory/scripts/demo_hillclimb.py on Modal; see docs/demo-hillclimb.md.",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("report", help="regenerate the HTML monitoring dashboard (report.html)").set_defaults(fn=cmd_report)
